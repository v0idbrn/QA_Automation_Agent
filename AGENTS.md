...
- `gui.py` + `QA_Agent.bat` — tkinter control center on top of the CLI (same commands, zero new execution paths); `build_command()` is the pure, tested argv composer. The bat autodetects `.venv`, guards tkinter, and launches detached via `pythonw`. The results panel reads the newest valid `audit_report.json` (newest-first fallback past corrupt artifacts) and renders findings by severity + quality-gate verdict. Frozen (`sys.frozen`): the exe self-spawns with a subcommand (`agent_invocation`) and `gui.main` dispatches agent subcommands to `main.main`, so the windowed exe doubles as the headless runner.
- `qa_agent.spec` + `scripts/build_exe.py` — PyInstaller onedir bundle (`dist/QA_Agent/`): windowed `QA_Agent.exe` + console `qa-agent.exe`, both with bundled report templates (`_MEIPASS`-aware resolution in `core.report_builder._default_template_dir`) and the Playwright driver; pytest excluded. `install-browsers` subcommand downloads browser engines on the target machine.
